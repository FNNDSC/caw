...
class UploadedFiles(ConnectedResource, Iterable):
    ...
    def __init__(self, url: str, session: requests.Session):
        ...
        try:
            ...
            raise InvalidFilesResourceUrlException(
                f"{e.response.status_code} error getting {self.url}"
            )
        # check given URL is a files collection resource
        if "count" not in self._initial_data:
            raise InvalidFilesResourceUrlException(
                f"{self.url} does not look like a files collection resource."
            )
        if self._initial_data["count"] > 0:
            try:
                ...
                raise InvalidFilesResourceUrlException(
                    f"{self.url} is not a files collection resource."
                )
