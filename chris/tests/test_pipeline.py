...
def test_freeze(mocker: MockerFixture):
    ...
    em = _MutablePluginTreeNode(
        s=session, piping=ep, params={ParameterName("this"): "isit"}
    )
    ...
    am = _MutablePluginTreeNode(
        s=session, piping=ap, params={ParameterName("bow"): "wow"}
    )
    ...
    et = PluginTree(
        s=session, piping=ep, default_parameters={ParameterName("this"): "isit"}
    )
    dt = PluginTree(s=session, piping=dp, default_parameters={})
    ct = PluginTree(s=session, piping=cp, default_parameters={}, children=(dt, et))
    bt = PluginTree(s=session, piping=bp, default_parameters={})
    at = PluginTree(
        s=session,
        piping=ap,
        default_parameters={ParameterName("bow"): "wow"},
        children=(bt, ct),
    )
